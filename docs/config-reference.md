...
| `prompt_hard_timeout_secs` | u64 | `1800` | Maximum inactivity during one prompt. Agent notifications reset the timer, so productive turns may run longer; human permission waits are excluded. The legacy key name is retained for config compatibility. |
