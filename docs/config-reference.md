...
| Variable | Default | Description |
|----------|---------|-------------|
| `OPENAB_RUNTIME_LOGIN_COMMAND` | _(empty)_ | Whitespace-separated argv (no shell) that `_openab/runtime/login` runs. It should print NDJSON objects on stdout; each is relayed to the operator as an `_openab/runtime/login/frame` notification and is never logged. Its stdin stays open for the lines the operator sends with `_openab/runtime/login/input` (for a flow that ends in a pasted code). Unset → the method reports that this runtime has no sign-in. |
...
| `OPENAB_RUNTIME_DISK_PATHS` | _(empty)_ | Comma-separated paths whose filesystems `_openab/runtime/state` reports in `usage.diskUsedBytes` / `usage.diskTotalBytes`, each filesystem counted once. Unset → both are `null`. |
